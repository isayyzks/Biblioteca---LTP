...
while (opcao !== 0) {
  ...
  switch (opcao) {
    ...
    case 3: {
      ...
      const entradaIndice = prompt(
        'Digite o índice do livro a alterar: '
      );
      ...
      if (novoGenero !== '') {
        livros[indice].genero = novoGenero;
      }
    ...
    case 4: {
      ...
      const contagemPorGenero =
        aux.contarPorGenero(livros);

      const media =
        aux.mediaNumPaginas(livros);

      console.log('\n-- Estatísticas --');
      console.log('Contagem por gênero:');

      for (const genero in contagemPorGenero) {
        console.log(
          `${genero}: ${contagemPorGenero[genero]}`
        );
      }

      console.log(`Média de páginas: ${media}`);
      break;
    }
